...
*Setup instructions for a Gitian build of Vaizon.*

Gitian is the deterministic build process that is used to build the Vaizon CLI
...
```bash
cp vaizon/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```bash
./gitian-build.py --setup vaizon-user x.x.x
```

Where `vaizon-user` is your Github name and `x.x.x` is the version tag you want to build.
If you are using docker, run it with:

```bash
./gitian-build.py --setup --docker vaizon-user v3.0.4
```
...
```bash
git clone git@github.com:vaizon-project/gitian.sigs.git
git remote add vaizon-user git@github.com:vaizon-user/gitian.sigs.git
```
...
```bash
./gitian-build.py --detach-sign --no-commit -b vaizon-user v3.0.4
```
...
```bash
export NAME=vaizon-user
export VERSION=v3.0.4
gpg --output $VERSION-linux/$NAME/vaizon-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/vaizon-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/vaizon-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/vaizon-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/vaizon-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/vaizon-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[vaizon-project/gitian.sigs](https://github.com/vaizon-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/vaizon-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/vaizon-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vaizon-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/vaizon-project/vaizon -b <branch> <commit hash>
```
