...
class SNNetwork:
    def __init__(self, datadir, *, binpath='../../build/bin', sns=20, nodes=3):
        ...
        nodeopts = dict(vaizon=self.binpath+'/vaizon', datadir=datadir)
        ...
        for name in ('Alice', 'Bob', 'Mike'):
            self.wallets.append(Wallet(
                ...
                name=name,
                rpc_wallet=self.binpath+'/vaizon-wallet-rpc',
                datadir=datadir))
        ...
        vprint("Starting new vaizon service nodes with RPC on {} ports".format(self.sns[0].listen_ip), end="")
        for sn in self.sns:
            vprint(" {}".format(sn.rpc_port), end="", flush=True, timestamp=False)
            sn.start()
        vprint(timestamp=False)
        vprint("Starting new regular vaizon nodes with RPC on {} ports".format(self.nodes[0].listen_ip), end="")
        for d in self.nodes:
            vprint(" {}".format(d.rpc_port), end="", flush=True, timestamp=False)
            d.start()
        vprint(timestamp=False)

        vprint("Waiting for all vaizon's to get ready")
        for d in self.all_nodes:
            d.wait_for_json_rpc("get_info")

        vprint("Vaizons are ready. Starting wallets")
        ...
        vprint("Sending fake vaizonnet/ss pings")
        for sn in self.sns:
            sn.ping()
...
def chuck(net):
    ...
    chuck = Wallet(node=net.nodes[0], name='Chuck', rpc_wallet=net.binpath+'/vaizon-wallet-rpc', datadir=net.datadir)
    chuck.ready(wallet="chuck")

    hidden_node = Daemon(vaizon=net.binpath+'/vaizon', datadir=net.datadir)
    bridge_node = Daemon(vaizon=net.binpath+'/vaizon', datadir=net.datadir)
    for x in (4, 7):
        bridge_node.add_peer(net.all_nodes[x])
    bridge_node.add_peer(hidden_node)
    hidden_node.add_peer(bridge_node)

    vprint("Starting new chuck vaizon bridge node with RPC on {}:{}".format(bridge_node.listen_ip, bridge_node.rpc_port))
    bridge_node.start()
    bridge_node.wait_for_json_rpc("get_info")
    net.sync(extra_nodes=[bridge_node], extra_wallets=[chuck])

    vprint("Starting new chuck vaizon hidden node with RPC on {}:{}".format(hidden_node.listen_ip, hidden_node.rpc_port))
    hidden_node.start()
    ...
    chuck.refresh()
    chuck.stop()
    chuck.hidden = Wallet(node=hidden_node, name='Chuck (hidden)', rpc_wallet=net.binpath+'/vaizon-wallet-rpc', datadir=net.datadir)
