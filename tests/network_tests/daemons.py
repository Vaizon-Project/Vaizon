...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'vaizon@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [vaizon] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/vaizon-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=vaizon.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, vaizonnet=True):
        """Sends fake storage server and vaizonnet pings to the running vaizon"""
        if storage:
            self.json_rpc("storage_server_ping", { "version_major": 9, "version_minor": 9, "version_patch": 9 })
        if vaizonnet:
            self.json_rpc("vaizonnet_ping", { "version": [9,9,9] })
