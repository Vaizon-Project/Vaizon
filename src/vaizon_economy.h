...
constexpr uint64_t COIN                       = (uint64_t)1000000000; // 1 VAIZON = pow(10, 9)
...
constexpr uint64_t BLOCK_REWARD_HF15      = 20 * COIN;
constexpr uint64_t MINER_REWARD_HF15      = BLOCK_REWARD_HF15 * 10 / 100;
constexpr uint64_t SN_REWARD_HF15         = BLOCK_REWARD_HF15 * 90 / 100;


// New (HF16+) money supply parameters (tentative - HF16 not yet scheduled)
constexpr uint64_t BLOCK_REWARD_HF16      = 20 * COIN  /* TODO - see below */;
constexpr uint64_t SN_REWARD_HF16         = BLOCK_REWARD_HF16 * 90 / 100;

// TODO: For now we add 1 extra atomic vaizon to the HF16 block reward, above; ultimately with pulse
...
constexpr uint64_t MINER_REWARD_HF16 = BLOCK_REWARD_HF16 * 10 / 100;

static_assert(MINER_REWARD_HF15 + SN_REWARD_HF15  == BLOCK_REWARD_HF15, "math fail");
static_assert(MINER_REWARD_HF16 + SN_REWARD_HF16  == BLOCK_REWARD_HF16, "math fail");
...
constexpr uint64_t burn_needed(uint8_t /*hf_version*/, mapping_type type)
{
  ...
  switch (type)
  {
    ...
    case mapping_type::vaizonnet_1year: /* FALLTHRU */
    ...
    case mapping_type::vaizonnet_2years: result = 40 * COIN; break;
    case mapping_type::vaizonnet_5years: result = 80 * COIN; break;
    case mapping_type::vaizonnet_10years: result = 120 * COIN; break;
  }
  return result;
}
}; // namespace lns
