...
#define VAIZON_DEFAULT_LOG_CATEGORY "lns"
...
std::string lns_extra_string(cryptonote::network_type nettype, cryptonote::tx_extra_vaizon_name_system const &data)
{
...
sqlite3 *init_vaizon_name_system(char const *file_path)
{
...
uint64_t expiry_blocks(cryptonote::network_type nettype, mapping_type type, uint64_t *renew_window)
{
  ...
  if (is_vaizonnet_type(type))
  {
    ...
    if (type == mapping_type::vaizonnet_1year)        result = BLOCKS_EXPECTED_IN_YEARS(1);
    else if (type == mapping_type::vaizonnet_2years)  result = BLOCKS_EXPECTED_IN_YEARS(2);
    else if (type == mapping_type::vaizonnet_5years)  result = BLOCKS_EXPECTED_IN_YEARS(5);
    else if (type == mapping_type::vaizonnet_10years) result = BLOCKS_EXPECTED_IN_YEARS(10);
...
bool validate_lns_name(mapping_type type, std::string name, std::string *reason)
{
  ...
  VAIZON_DEFER { if (reason) *reason = err_stream.str(); };

  bool const is_vaizonnet = is_vaizonnet_type(type);
  size_t max_name_len   = 0;

  if (is_vaizonnet)                         max_name_len = lns::VAIZONNET_DOMAIN_NAME_MAX;
  ...
  {
    // VAIZONNET
    // Domain has to start with an alphanumeric, and can have (alphanumeric or hyphens) in between, the character before the suffix <char>'.vaizon' must be alphanumeric followed by the suffix '.vaizon'
    // ^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.vaizon$

    if (check_condition(name == "localhost.vaizon", reason, "LNS type=", type, ", specifies mapping from name->value using protocol reserved name=", name))
      return false;
    ...
    char const SHORTEST_DOMAIN[] = "a.vaizon";
    if (check_condition((name.size() < static_cast<int>(vaizon::char_count(SHORTEST_DOMAIN))), reason, "LNS type=", type, ", specifies mapping from name->value where the name is shorter than the shortest possible name=", SHORTEST_DOMAIN, ", given name=", name))
      return false;

    // Must end with .vaizon
    char const SUFFIX[]     = ".vaizon";
    char const *name_suffix = name.data() + (name.size() - vaizon::char_count(SUFFIX));
    if (check_condition((memcmp(name_suffix, SUFFIX, vaizon::char_count(SUFFIX)) != 0), reason, "LNS type=", type, ", specifies mapping from name->value where the name does not end with the domain .vaizon, name=", name))
      return false;

    // Characted preceeding suffix must be alphanumeric
    char const *char_preceeding_suffix = name_suffix - 1;
    if (check_condition(!char_is_alphanum(char_preceeding_suffix[0]), reason, "LNS type=", type ,", specifies mapping from name->value where the character preceeding the <char>.vaizon is not alphanumeric, char=", char_preceeding_suffix[0], ", name=", name))
      return false;
...
bool validate_mapping_value(cryptonote::network_type nettype, mapping_type type, std::string const &value, mapping_value *blob, std::string *reason)
{
  ...
  {
    int max_value_len = 0;
    if (is_vaizonnet_type(type))              max_value_len = (VAIZONNET_ADDRESS_BINARY_LENGTH * 2);
  ...
  else if (is_vaizonnet_type(type))
  {
    if (check_condition(value.size() != 52, reason, "The vaizonnet value=", value, ", should be a 52 char base32z string, length=", value.size()))
      return false;
...
bool validate_encrypted_mapping_value(mapping_type type, std::string const &value, std::string *reason)
{
  ...
  if (is_vaizonnet_type(type)) max_value_len              += VAIZONNET_ADDRESS_BINARY_LENGTH;
...
static bool validate_against_previous_mapping(lns::name_system_db &lns_db, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_vaizon_name_system const &lns_extra, std::string *reason = nullptr)
{
  std::stringstream err_stream;
  VAIZON_DEFER { if (reason && reason->empty()) *reason = err_stream.str(); };
  ...
  if (mapping)
  {
    ...
    {
      if (check_condition(is_vaizonnet_type(lns_extra.type) && !mapping.active(lns_db.network_type(), blockchain_height), reason, tx, ", ", lns_extra_string(lns_db.network_type(), lns_extra), " TX requested to update mapping that has already expired"))
        return false;
      ...
      // Validate signature
      {
        ...
        crypto::hash hash = tx_extra_signature_hash(value,
                                                    ...
                                                    expected_prev_txid);
        if (check_condition(!hash, reason, tx, ", ", lns_extra_string(lns_db.network_type(), lns_extra), " unexpectedly failed to generate signature hash, please inform the Vaizon developers"))
          return false;
    ...
    {
      if (!is_vaizonnet_type(lns_extra.type))
      {
        if (check_condition(true, reason, tx, ", ", lns_extra_string(lns_db.network_type(), lns_extra), " non-vaizonnet entries can NOT be renewed, mapping already exists with name_hash=", mapping.name_hash, ", owner=", mapping.owner.to_string(lns_db.network_type()), ", type=", mapping.type))
          return false;
      }
      ...
      {
        // Vaizonnet entry expired i.e. it's no longer active. A purchase for this name is valid
...
bool name_system_db::validate_lns_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_vaizon_name_system *lns_extra, std::string *reason)
{
  // -----------------------------------------------------------------------------------------------
  // Pull out LNS Extra from TX
  // -----------------------------------------------------------------------------------------------
  cryptonote::tx_extra_vaizon_name_system lns_extra_;
  {
    if (!lns_extra) lns_extra = &lns_extra_;
    if (check_condition(tx.type != cryptonote::txtype::vaizon_name_system, reason, tx, ", uses wrong tx type, expected=", cryptonote::txtype::vaizon_name_system))
      return false;

    if (check_condition(!cryptonote::get_vaizon_name_system_from_tx_extra(tx.extra, *lns_extra), reason, tx, ", didn't have vaizon name service in the tx_extra"))
      return false;
  }
  ...
  // -----------------------------------------------------------------------------------------------
  {
    ...
    if (burn != burn_required)
    {
      ...
      if (check_condition(true, reason, tx, ", ", lns_extra_string(nettype, *lns_extra), " burned ", over_or_under, "vaizon=", burn, ", require=", burn_required))
        return false;
    }
  }
...
bool encrypt_mapping_value(std::string const &name, mapping_value const &value, mapping_value &encrypted_value)
{
  ...
  static_assert(mapping_value::BUFFER_SIZE >= VAIZONNET_ADDRESS_BINARY_LENGTH    + crypto_secretbox_MACBYTES, "Value blob assumes the largest size required, all other values should be able to fit into this buffer");
...
static int64_t add_or_get_owner_id(lns::name_system_db &lns_db, crypto::hash const &tx_hash, cryptonote::tx_extra_vaizon_name_system const &entry, lns::generic_owner const &key)
{
  int64_t result = 0;
...
static bool add_lns_entry(lns::name_system_db &lns_db, uint64_t height, cryptonote::tx_extra_vaizon_name_system const &entry, crypto::hash const &tx_hash)
{
  // -----------------------------------------------------------------------------------------------
...
bool name_system_db::add_block(const cryptonote::block &block, const std::vector<cryptonote::transaction> &txs)
{
  ...
  if (block.major_version >= cryptonote::network_version_15_lns)
  {
    ...
    {
      if (tx.type != cryptonote::txtype::vaizon_name_system)
        continue;

      cryptonote::tx_extra_vaizon_name_system entry = {};
...
static bool get_txid_lns_entry(cryptonote::Blockchain const &blockchain, crypto::hash txid, cryptonote::tx_extra_vaizon_name_system &extra)
{
  ...
  return cryptonote::get_vaizon_name_system_from_tx_extra(txs[0].extra, extra);
}
...
struct lns_update_history
{
  ...
  void     update(uint64_t height, cryptonote::tx_extra_vaizon_name_system const &lns_extra);
  uint64_t newest_update_height() const;
};

void lns_update_history::update(uint64_t height, cryptonote::tx_extra_vaizon_name_system const &lns_extra)
{
  if (lns_extra.field_is_set(lns::extra_field::encrypted_value))
...
static std::vector<replay_lns_tx> find_lns_txs_to_replay(cryptonote::Blockchain const &blockchain, lns::mapping_record const &mapping, uint64_t blockchain_height)
{
  ...
  */
  ...
  {
    cryptonote::tx_extra_vaizon_name_system curr_lns_extra = {};
...
bool name_system_db::save_mapping(crypto::hash const &tx_hash, cryptonote::tx_extra_vaizon_name_system const &src, uint64_t height, int64_t owner_id, int64_t backup_owner_id)
{
  if (!src.is_buying())
    return false;
