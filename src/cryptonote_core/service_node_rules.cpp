...
// TODO(vaizon): Move to vaizon_economy, this will also need access to vaizon::exp2
uint64_t get_staking_requirement(cryptonote::network_type m_nettype, uint64_t height, uint8_t hf_version)
{
  ...
  if (hf_version >= cryptonote::network_version_13_enforce_checkpoints)
  {
    ...
    constexpr uint64_t LAST_HEIGHT      = heights[vaizon::array_count(heights) - 1];
    constexpr uint64_t LAST_REQUIREMENT = lsr    [vaizon::array_count(lsr) - 1];
    if (height >= LAST_HEIGHT)
        return LAST_REQUIREMENT;

    size_t i = 0;
    for (size_t index = 1; index < vaizon::array_count(heights); index++)
    {
  ...
  {
    base     = 15000 * COIN;
    variable = (25007.0 * COIN) / vaizon::exp2(height_adjusted/129600.0);
  }
  else
  {
    base      = 10000 * COIN;
    variable  = (35000.0 * COIN) / vaizon::exp2(height_adjusted/129600.0);
  }
