...
if len(sys.argv) < 3:
    ...
    if m:
        vaizonrpc = "http://{}:{}/json_rpc".format(m.group(1), m.group(2))
        r = requests.post(vaizonrpc, json={"jsonrpc":"2.0", "id":0, "method":"get_service_node_privkey"}).json()
        if "result" in r and "service_node_x25519_privkey" in r["result"]:
            ...
            print("Loaded x25519 keys (pub: {}) from vaizon @ {}".format(hexstr(x_key.public_key), sys.argv[1]))
        else:
            x_key = PrivateKey.generate()
            print("Generated x25519 key {} (vaizon @ {} did not return SN privkeys)".format(hexstr(x_key.public_key), sys.argv[1]))
    else:
        print("Error: {} does not look like a valid vaizon RPC host:port value".format(sys.argv[1]), sys.stderr)
        badargs = True
...
if direct:
    ...
    r = requests.post(vaizonrpc, json={"jsonrpc":"2.0", "id":0, "method":"get_service_nodes", "params": {
